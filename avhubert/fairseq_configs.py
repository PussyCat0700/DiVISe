...
# Fairseq's ChoiceEnum does not work with multiprocessing
# So We're replacing them with vanilla lists.
# Note that these lists are just here for reference. The values in config will not be checked.
...
# I really don't want to paste Wav2Vec2Config here, 
# but I must workaround ChoiceEnum too here.
@dataclass
class Wav2Vec2Config(FairseqDataclass):
    layer_type: str = field(
        default="transformer", metadata={"help": "layer type in encoder"}
    )  # Should be in LAYER_TYPE_CHOICES
    # dropouts
    dropout: float = field(
...
class AVHubertConfig(Wav2Vec2Config):
    ...
    )  # Should be in EXTRACTOR_MODE_CHOICES
    ...
    )  # Should be in fairseq.utils.get_available_activation_fns()
    ...
    mask_selection: str = field(
        default="static", metadata={"help": "how to choose mask length"}
    )  # Should be in MASKING_DISTRIBUTION_CHOICES
    mask_other: float = field(
        default=0,
        metadata={
