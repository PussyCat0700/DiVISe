...
def align_mouth(fids, rank):
    ...
    for filename_idx, filename in enumerate(tqdm(fids)):
        ...
        if not os.path.isfile(video_pathname) or not os.path.isfile(landmarks_pathname):
            with open('unable_align.list', 'a+') as f:
                f.write(landmarks_pathname+"\n")
...
def on_error(err):
    str_err = str(err)
    print(str_err)
    with open("align_error.err", 'a+') as f:
...
if __name__ == '__main__':
    ...
    for rank in range(args.nshard):
        pool.apply_async(align_mouth, args=(fids, rank), error_callback=on_error,)
    pool.close()
    pool.join()
    print('Done.')
