...
def on_error(err):
    str_err = str(err)
    print(str_err)
    with open("detect_landmark_error.err", 'a+') as f:
        f.writelines(str_err+'\n')
...
if __name__ == '__main__':
    ...
    import skvideo.io
    if args.nshard == 1:
        detect_face_landmarks(args.face_predictor, args.cnn_detector, args.root, args.landmark, args.manifest, 0, args.nshard)
    else:
        pool = multiprocessing.Pool(args.nshard)
        for rank in range(args.nshard):
            pool.apply_async(detect_face_landmarks, args=(args.face_predictor, args.cnn_detector, args.root, args.landmark, args.manifest, rank, args.nshard), error_callback=on_error,)
        pool.close()
        pool.join()
