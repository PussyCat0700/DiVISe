...
from .audio import preprocess_wav, wav_to_mel_spectrogram   # We want to expose this function from here
from matplotlib import cm
from pathlib import Path
import numpy as np
import torch
from contrastive.metrics import calc_cosine_similarity
...
def load_model(weights_fpath: Path, device=None):
    ...
    _model.eval()
    print(f"Loaded encoder {weights_fpath.name} on device {_model.device} trained to step {checkpoint['step']}")
...
def embed_frames_batch(frames_batch):
    ...
    frames = torch.from_numpy(frames_batch).to(_model.device)
...
def compute_similarity(waveforms, wav_padding_masks=None, max_audio_sample_size=None, pad_audio=True):
    ...
    if pad_audio:
        audio_embeddings = []
        for waveform, padding_mask in zip(waveforms, wav_padding_masks):
            x = waveform.masked_select(padding_mask)
            x = x.detach().cpu().numpy()
            audio_embedding = embed_utterance(x)
    ...
    # [2*B, C] -> [2, B, C]
    audio_embeddings = audio_embeddings.reshape(2, -1, audio_embeddings.shape[-1])
    similarity = calc_cosine_similarity(audio_embeddings)
    return similarity
